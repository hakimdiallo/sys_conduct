...
struct conduct *conduct_create(const char *name, size_t a, size_t c){
  struct conduct* cond;
  if(name==NULL){
    cond = (struct conduct*) mmap(NULL, sizeof(struct conduct *), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if(cond == MAP_FAILED){
  ...
  memset(cond, 0, (sizeof(struct conduct)));
  ...
  cond->buffer=(void*)malloc(sizeof(void)*c);
...
ssize_t conduct_read(struct conduct *c, void *buf, size_t count){
  ...
  //printf("Thread lecture prend le verrou\n");
    ...
    if (m == *c->current_size) {
      ...
      memset(tmp, 0, *c->c);
      memcpy(tmp, c->buffer+m, *c->current_size - m);
      memset(c->buffer, 0, *c->c);
      memcpy(c->buffer, tmp, *c->current_size - m);
    }
//    printf("%d\n", (int)*c->current_size);
