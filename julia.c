...
static int julia(double complex z, double complex c) {
  int i = 0;
...
static void paintit(struct twocons *cons, cairo_t *cr, int repcount) {
...
static void set_title() {
  char buf[100];
  snprintf(buf, 100, "Julia %lf + %lfi", creal(julia_c), cimag(julia_c));
...
int main(int argc, char **argv) {
  ...
  g_signal_connect(window, "button_press_event", G_CALLBACK(button_callback),
                   NULL);
  set_title();
